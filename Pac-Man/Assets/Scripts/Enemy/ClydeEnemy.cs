...
public class ClydeEnemy : MonoBehaviour
{
    [SerializeField]
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            if (this.gameObject.transform.localScale.magnitude <= new Vector3(-0.59f, -4f, -2.5f).magnitude)
            {
                GM.ActualScore += 100;
                UI.UpdateScore();
            ...
            {
                this.gameObject.transform.localScale -= new Vector3(-0.59f, -4f, -2.5f);
            }
        }
